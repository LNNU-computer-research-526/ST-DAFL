...
parser.add_argument('--mb_rate', type=float, default=0.1)
...
teacher = torch.load(opt.teacher_dir + 'teacher-cifar10').cuda()
teacher.eval()
...
def fill_signal(dp):
    ...
    threshold = temp[opt.batch_size - int(opt.mb_rate * opt.batch_size)]
...
def proc_bad():
    ...
    return
# def cal_spec_loss():
#     global memory_bank, temp_x, bad_imgs
#     import random
#     start = 0
#     end = len(signal)
...
memory_bank = []
dot_product = []
mb_len = opt.batch_size - int(opt.mb_rate * opt.batch_size)
for epoch in range(opt.n_epochs):
    ...
    for num in range(120):
        ...
        lowest_loss_bg = 10000.0
        temp_x = list(temp_x.cpu().detach())
#         if signal.count(0) > signal.count(1):
#             proc_bad()
        for i in range(5):
                bad_o = teacher(torch.stack(random.sample(temp_x, len(memory_bank))))
                great_o = teacher(torch.stack(memory_bank))
                loss_bg = kdloss(bad_o.detach(), great_o.detach())
                if loss_bg < lowest_loss_bg:
                    lowest_loss_bg = loss_bg

        great_imgs = torch.stack(random.sample(memory_bank, mb_len))
        ...
        loss = loss_one_hot * opt.oh + loss_information_entropy * opt.ie + loss_activation * opt.a - lowest_loss_bg * 0.01
    ...
    print('bgloss:',lowest_loss_bg.item())
    with torch.no_grad():
    ...
    if accr > accr_best:
        torch.save(net, opt.output_dir + 'mine_ABD_student_cifar10')
        accr_best = accr
print('acc_best:', accr_best)
